...
int exec() {
  int cont = 1;
  int addr, val;
   static const void* insOpCodeJumpTable[16] = {&&C0, &&C1, &&C2, &&C3, &&C4, &&DEFAULT, &&C6,
        &&C7, &&C8, &&C9, &&C10, &&C11, &&C12, &&C13, &&C14, &&C15};
   static const void* insOp0JumpTable[16] = { &&L0, &&L1, &&L2, &&L3, &&L4, &&L5, &&L6,
        &&L7, &&L15, &&DEFAULT1};

        if (insOpCode < 0 || insOpCode == 5 || insOpCode > 15) {
          goto DEFAULT;
        }

        goto *insOpCodeJumpTable[insOpCode];
...
// DEFAULT:   printf ("Illegal  instruction: pc=0x%x opCode=0x%x\n", pc, insOpCode);

C6:     // ALU ................... 6-sd

        if (insOp0 < 0 || (insOp0 > 8 && insOp0 < 15) || insOp0 > 15)
          goto DEFAULT1;

        if (insOp0 == 15)
          goto L15;

        goto *insOp0JumpTable[insOp0];
