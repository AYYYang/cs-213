...
uthread_sem_t need_items;
uthread_sem_t have_items;
uthread_sem_t lock;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(have_items);
    uthread_sem_wait(lock);
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(need_items);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(need_items);
    uthread_sem_wait(lock);
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;  
    uthread_sem_signal(have_items);
    uthread_sem_signal(lock);
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  need_items = uthread_sem_create(0);
  have_items = uthread_sem_create(MAX_ITEMS);
