 ...
 */
void list_delete (struct list* l) { // needs to keep reference count before free(l) is called
  for (struct element *e = l->head, *n; e != NULL; e = n) {
    n = element_get_next (e);
    list_delete_element (l, e); // calls delete element, this should be the problem
  }
  free (l);
}
